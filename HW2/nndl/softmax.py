...
class Softmax(object):
    ...
    """
        ...
        scores -= np.max(scores, axis=1, keepdims=True)  # safe softmax
        exp_scores = np.exp(scores)
        pr = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)
        correct_log_pr = -np.log(pr[np.arange(X.shape[0]), y])
        loss = np.sum(correct_log_pr) / X.shape[0]
    ...
    """
        ...
        scores -= np.max(scores, axis=1,
                         keepdims=True)  # for numerical stability
        exp_scores = np.exp(scores)
