...
class KNN(object):
    ...
    """
        ...
        dists = np.reshape(np.sum(X**2, axis=1), [num_test, 1]) + np.sum(
            self.X_train**2, axis=1) - 2 * np.dot(X, self.X_train.T)
    ...
    def compute_Linf_distances_vectorized(self, X):
        ...
        dists = np.max(np.abs(X[:, np.newaxis] - self.X_train), axis=2)
